...
13. Click New folder in the sidebar: the row becomes a focused field. Escape restores the
    button and returns focus to it, clicking away with the field empty does the same, and
    a name plus Enter creates the folder and leaves the button back in place.
14. Hover a card: the pin appears at its top right — filled on a pinned note — and
